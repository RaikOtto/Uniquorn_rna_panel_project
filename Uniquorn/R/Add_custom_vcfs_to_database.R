...
#'  
#' @param vcf_input_files a character vector containing paths to the
...
#' @import DBI data.table foreach doParallel
#' @usage 
...
add_custom_vcf_to_database = function(
  ...
  test_mode = FALSE
    ...
    index_vcf = lapply(vcf_input_files, function(x) check_files(x))
    if (sum(unlist(index_vcf)) < length(vcf_input_files)){
        warning("Could not find the following VCF file(s), skipping: \n", 
                paste(vcf_input_files[!unlist(index_vcf)], collapse = "\n"))
    }
