...
all_cls = c(c[grepl("COSMIC",c[,1]),1], c[grepl("CCLE",c[,1]),1],c[grepl("CELLMINER",c[,1]),1]  )
...
missing_cls = known_cls[ !(known_cls %in% identifier )]
missing_cls = sapply( missing_cls, paste, sep = "_", "CUSTOM")

all_cls = as.character( c(all_cls, missing_cls))
all_cls = cbind( all_cls, rep("", length(all_cls)) )
...
make_name_comparable = function( replace_name ){
  ...
  replace_name = as.character( str_replace_all( string = replace_name, pattern = "(_COSMIC)|(_CCLE)|(_CELLMINER)", "" ) )
  replace_name = as.character( str_replace_all( string = replace_name, pattern = "([^A-Z0-9])*", "" ) )
  
  return( replace_name )
}

stripped_names = sapply( all_cls[,1], FUN = make_name_comparable )
...
all_cls[ , 2] = sapply( seq( length( all_cls[,1] ) ), FUN = identify_similar_names )
...
naked_merge = lapply( FUN = str_replace_all, merged_sim_rel, pattern = "(_CCLE*)|(_COSMIC*)|(_CELLMINER*)", replacement = ""  )
