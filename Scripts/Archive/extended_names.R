library("stringr")
source("~/Dropbox/PhD/Uniquorn_project/Scripts/Benchmark.R")
library("Uniquorn")

# load extension list

known_pairs = read.table("~/Uniquorn_data/benchmark_vcf_files/known_relationships.tab",sep ="\t", header = F, fill=T, col.names = c("Cl1","Cl2","Relationship","URL","",""))
known_id_table = known_pairs[,seq(2)]

id_pairs_1 = paste( known_id_table$Cl1, known_id_table$Cl2, sep ="_" )
id_pairs_2 = paste( known_id_table$Cl2, known_id_table$Cl1, sep ="_" )
...
for (i in seq(length(cellminer_cls))){
  
  CL = cellminer_cls[i]
  for (j in seq(dim(known_pairs)[1]) ){
    
    if ( ( CL == known_pairs[j,1] ) | ( CL == known_pairs[j,2]) )
      cellminer_cls[i] = paste( known_pairs[j,1], known_pairs[j,2], sep = "_" )
  }
}
for (i in seq(length(ccle_cls))){
  
  CL = ccle_cls[i]
  for (j in seq(dim(known_pairs)[1]) ){
    
    if ( ( CL == known_pairs[j,1] ) | ( CL == known_pairs[j,2]) )
      ccle_cls[i] = paste( known_pairs[j,1], known_pairs[j,2], sep = "_" )
  }
}
for (i in seq(length(cosmic_cls))){
  
  CL = cosmic_cls[i]
  for (j in seq(dim(known_pairs)[1]) ){
    
    if ( ( CL == known_pairs[j,1] ) | ( CL == known_pairs[j,2]) )
      cosmic_cls[i] = paste( known_pairs[j,1], known_pairs[j,2], sep = "_" )
  }
}

length( c( cellminer_cls, ccle_cls, cosmic_cls ) ) # 1989 CL images
length( unique( c( cellminer_cls, ccle_cls, cosmic_cls ) ) ) # pair-wisely non related CCL samples
...
length(intersect_cosmic_ccle) # 627
length(intersect_cosmic_cellminer) # 55
length(intersect_ccle_cellminer) # 40

# real world benchmark integration

benchmark_res_t = read.table("~/Uniquorn_data/benchmark_vcf_files/Benchmark_results_regularized/_1__Relaxed_results.tab",sep ="\t", header =T)
